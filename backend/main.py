...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Инициализация синхронизации времени с биржей и серверами точного времени
        time_sync = ExchangeTimeSync()
        await time_sync.start()
        logger.info("Синхронизация времени UTC запущена")
        ...
        # Инициализация менеджера алертов С синхронизацией времени UTC
        ...
        if trading_pairs:
            ...
            logger.info("Система успешно запущена с синхронизацией времени UTC!")
        else:
...
async def get_time_info():
    """Получить информацию о времени UTC"""
    try:
        if time_sync:
            # Возвращаем информацию о синхронизации UTC времени
            sync_status = time_sync.get_sync_status()
            logger.info(f"API /api/time: Возвращаем UTC время. serverTime={sync_status['serverTime']}")
            return sync_status
        else:
        ...
        return {
            ...
            "utc_time": current_time_ms,
            "utc_time_iso": datetime.utcnow().isoformat() + 'Z',
            "sync_method": "emergency_fallback",
            "time_offset_ms": 0,
            "status": "error",
            "error": str(e)
        }
...
async def update_settings(settings: dict):
    ...
    try:
        ...
        if time_sync and 'time_sync' in settings:
            sync_method = settings['time_sync'].get('sync_method', 'auto')
