...
class DatabaseManager:
    ...
    async def save_alert(self, alert_data: Dict) -> int:
        ...
        try:
            ...
            alert_timestamp_ms = int(alert_data['timestamp']) if isinstance(alert_data['timestamp'],
                                                                            (int, float)) else int(
                datetime.fromisoformat(str(alert_data['timestamp']).replace('Z', '+00:00')).timestamp() * 1000)
            ...
            if alert_data.get('close_timestamp'):
                if isinstance(alert_data['close_timestamp'], (int, float)):
                    ...
                    close_timestamp_ms = int(datetime.fromisoformat(
                        str(alert_data['close_timestamp']).replace('Z', '+00:00')).timestamp() * 1000)
